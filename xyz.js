...
(function () {
  ...
  const LocalAI = {
    ...
    convertUnits(value, from, to) {
      ...
      const conv = { 'km-mi': v=>v0.621371,'mi-km': v=>v1.60934,'kg-lb': v=>v2.20462,'lb-kg': v=>v0.453592,'c-f': v=>(v9/5)+32,'f-c': v=>(v-32)5/9,'usd-inr': v=>v*83.5,'inr-usd': v=>v/83.5 };
    ...
    respondGeneralKnowledge(lower) {
      ...
      for (const item of kb) {
        if (item.keys.every(k => lower.includes(k))) return â€œ**${item.keys.join(' ')}**: ${item.answer}â€;
      }
