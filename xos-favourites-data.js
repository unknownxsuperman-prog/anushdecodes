...
window.XOS_FAV = {
  ...
  // NEW: Initialize with the example data from the comment
  initExampleData() {
    const store = this._loadStore();
    // Only write if the store is empty, or if we want to force overwrite we can remove the condition
    if (Object.keys(store).length === 0) {
      ...
      this._saveStore(store);
      console.log('Example favourites data seeded.');
    } else {
      console.log('Store already has data; skipping example seed.');
    }
  }
};

// Automatically seed example data when script loads (only if store empty)
XOS_FAV.initExampleData();
