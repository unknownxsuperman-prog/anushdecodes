 ...
 * Integrates: college-data.js · branches.js
...
// ========== KCET PREDICTOR with college-data.js + branches.js ==========

// All categories from college-data.js
const KCET_CATEGORIES = ['1H','1KH','1RH','2AH','2AKH','2ARH','2BH','2BKH','2BRH','3AH','3AKH','3ARH','3BH','3BKH','3BRH','GMH','GMKH','GMRH','SCH','SCKH','SCRH','STH','STKH','STRH'];

const CATEGORY_LABELS = {
  '1H':'GM-1 (Hindu)','1KH':'GM-1 Kannada','1RH':'GM-1 Rural',
  '2AH':'2A Hindu','2AKH':'2A Kannada','2ARH':'2A Rural',
  '2BH':'2B Hindu','2BKH':'2B Kannada','2BRH':'2B Rural',
  '3AH':'3A Hindu','3AKH':'3A Kannada','3ARH':'3A Rural',
  '3BH':'3B Hindu','3BKH':'3B Kannada','3BRH':'3B Rural',
  'GMH':'GM Hindu','GMKH':'GM Kannada','GMRH':'GM Rural',
  'SCH':'SC Hindu','SCKH':'SC Kannada','SCRH':'SC Rural',
  'STH':'ST Hindu','STKH':'ST Kannada','STRH':'ST Rural',
};
...
function dishaCategorySelect(uid){
  const opts = KCET_CATEGORIES.map(c=>
    `<option value="${c}">${c} — ${CATEGORY_LABELS[c]||c}</option>`
  ).join('');
  return `<select id="cat-select-${uid}"
    style="width:100%;background:#000;border:1px solid #222;border-radius:10px;padding:12px 14px;
    color:#fff;font-family:'Space Grotesk',sans-serif;font-size:.82rem;outline:none;cursor:pointer;
...
// Find colleges from college-data.js using selected category + branch, rank ±2000
function dishaFindMatches(rank, branchQ, category){
  ...
  data.forEach(college=>{
    Object.keys(college.branches).forEach(bn=>{
      ...
      // if category selected, use that column; else search all non-null
      let checkCuts;
      ...
      if(diff>=-W&&diff<=W){
        matches.push({college:college.name,id:college.id,location:college.location,branch:bn,category:closest[0],cutoff:closest[1],diff});
      }
    });
  });
...
function dishaRenderMatches(matches){
  if(!matches.length) return `<div style="color:#444;font-size:.75rem;padding:10px 0;text-align:center;">No colleges found in ±2000 range.<br><span style="font-size:.65rem;">Try a different category or branch.</span></div>`;
...
function dishaGetCat(uid){return document.getElementById(`cat-select-${uid}`)?.value||'';}
...
window.dishaCalcEng=function(uid){
  ...
  res.innerHTML=`
    <div style="background:#000;border:1px solid #1a1a1a;border-radius:16px;padding:20px;text-align:center;margin-bottom:${branch?'16px':'0'};">
      ...
      ${cat?`<div style="margin-top:8px;font-size:.62rem;color:#555;letter-spacing:.06em;">Category · ${cat} — ${CATEGORY_LABELS[cat]||cat}</div>`:''}
...
window.dishaCalcNon=function(uid){
  ...
  res.innerHTML=`
    ...
    ${cat?`<div style="font-size:.62rem;color:#555;letter-spacing:.06em;margin-bottom:12px;">Category · ${cat} — ${CATEGORY_LABELS[cat]||cat}</div>`:''}
    ${branch?`<div style="font-size:.6rem;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:#444;margin-bottom:12px;">Colleges · ${branch}${cat?' · '+cat:''} · Rank ±2000</div>${dishaRenderMatches(matches)}`:''}
    <div onclick="launchKcetPredictor()" style="text-align:center;margin-top:12px;font-size:.65rem;color:#444;letter-spacing:.08em;text-transform:uppercase;cursor:pointer;padding:8px;">⟳ NEW PREDICTION</div>`;
  if(typeof window.scrollEsToBottom==='function') window.scrollEsToBottom();
};
